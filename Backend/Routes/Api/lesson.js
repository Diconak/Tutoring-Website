...
router.get("/", async (req, res, next) => {
  try {
    const lessons = await lessonModel
      .find()
      .sort({ langName: 1, lessonNumber: 1 })
      .exec();
    if (lessons) {
...
router.get("/langName/:langname", async (req, res, next) => {
...
router.delete("/deleteLesson/:lessonName", async (req, res, next) => {
  ...
  try {
    const lesson = await lessonModel
      .deleteOne({ lessonName: lessonname })
      .exec();
